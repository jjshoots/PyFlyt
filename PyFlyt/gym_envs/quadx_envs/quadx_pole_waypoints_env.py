...
class QuadXPoleWaypointsEnv(QuadXBaseEnv):
    ...
    def compute_term_trunc_reward(self) -> None:
        ...
        if not self.sparse_reward:
            self.reward += max(100.0 * self.waypoints.progress_to_next_target, 0.0)
            self.reward += 0.3 / self.waypoints.distance_to_next_target
            self.reward += 0.5 * (1.0 - self.pole.leaningness)
