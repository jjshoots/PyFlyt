...
def NormalInvGamma(gamma, nu, alpha, beta):
    ...
    """
    assert torch.all(nu > 0.0), "nu must be more than zero"
    assert torch.all(alpha > 1.0), "alpha must be more than one"
    assert torch.all(beta > 0.0), "beta must be more than zero"
