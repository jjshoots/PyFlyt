...
class VQVAE(nn.Module):
    ...
    def encoding_indices_to_encoding(self, encoding_indices):
        ...
        encoding = torch.zeros(*shape[:2], self.num_embedding, *shape[-2:]).to(
            encoding_indices.device
        )
        encoding = encoding.scatter(2, encoding_indices.unsqueeze(2), 1.0)
        return encoding
    ...
    def encoding_indices_to_quantized(self, encoding_indices):
        ...
        encoding_indices = encoding_indices.view(
            encoding_indices.shape[0] * encoding_indices.shape[1], -1
        )
